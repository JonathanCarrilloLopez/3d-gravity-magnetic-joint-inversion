...
double precision :: rms,error,alpha1,error1,error2,error3,rms1,rms2,rms3,diff_der
...
!Weighthin operators
WRITE(*,*) 'Weighthing operators'
DO i= 2,NC1
diff_der=malla(i,2)-malla(i-1,1)
if (diff_der==0) then
diff_der=0.0000001
end if

Dx(i,:) = Dx(i,:) / ((diff_der)/2)
enddo
...
diff_der=malla(k+(j-1)*(Nx-1)+(i-1)*(Nx-1)*(Ny-1),4)-malla(k+(j-2)*(Nx-1)+(i-1)*(Nx-1)*(Ny-1),3)
if (diff_der==0) then
diff_der=0.0000001
end if
...
diff_der=malla(k+(j-1)*(Nx-1)+(i-1)*(Nx-1)*(Ny-1),6)-malla(k+(j-1)*(Nx-1)+(i-2)*(Nx-1)*(Ny-1),5)
if (diff_der==0) then
diff_der=0.0000001
end if
